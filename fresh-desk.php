	...
	class Fresh_Desk_Official {
		...
		*/
		public function fdo_show_login_message() {
			...
			if ( !empty($this->freshdesk_options) && $this->freshdesk_options['freshdesk_enable_sso'] != 'checked' && sanitize_text_field( $_GET['action'] ) != null && sanitize_text_field($_GET['action']) == 'freshdesk-login' ){
		...
		*/
		public function fdo_settings_init() {
			...
			//Enable SSO
			add_settings_field( 'freshdesk_enable_sso', '', array($this, 'fdo_enable_sso_callback') , 'freshdesk-menu-handle', 'freshdesk_settings_section' );
		...
		/**
		* Function Name: fdo_enable_sso_callback
		* @return void
		*/
		public function fdo_enable_sso_callback() {
			...
			$this->fdo_sso_callback();
			
			//Remote login and Logout Urls
			$this->fdo_sso_urls_callback();
			echo '</div>';
		...
		*/
		...
		/**
		* Function Name: fdo_sso_callback
		* @return void
		*/
		public function fdo_sso_callback() {
			echo "<div class='freshdesk_sso_settings'><div class='info-title'>".__("Simple SSO shared secret")."</div><input class='fd_ui_element' id='freshdesk_sso_key' name='freshdesk_options[freshdesk_sso_key]' size='72' type='text' value='".esc_attr($this->freshdesk_options['freshdesk_sso_key'])."' />";
			echo '<div class="info-data fd_ui_element freshdesk_helpdesk_url">Enable SSO in your Freshdesk account, copy the <a href="'.esc_url($this->freshdesk_options["freshdesk_domain_url"]).'/admin/security" target="_blank" >SSO shared secret from this page</a> and paste it here.</div></div>';
		}

		/**
		* Function Name: fdo_sso_urls_callback
		* @return void
		*/
		public function fdo_sso_urls_callback() {
			...
			echo '<div class="info-data freshdesk_helpdesk_url">'.__("Copy the above <i>remote login URL</i> to your").' <a href="'.esc_url($this->freshdesk_options["freshdesk_domain_url"]).'/admin/security" target="_blank" >Freshdesk simple SSO settings.</a></div></li>';
			...
			echo '<div class="info-data freshdesk_helpdesk_url" id="freshdesk_redirect_url">'.__("Copy the above <i>remote logout URL</i> to your").' <a href="'.esc_url($this->freshdesk_options["freshdesk_domain_url"]).'/admin/security" target="_blank" >Freshdesk simple SSO settings.</a></div></li>';
