...
if(!class_exists('Fresh_Desk'))
{	
	/**
 	* Class Name: Fresh_Desk
 	*/
	class Fresh_Desk {
		...
		*/
		public function __construct()
		{
			...
			defined( 'FD_PLUGIN_URL' ) or define( 'FD_PLUGIN_URL', plugin_dir_url( __FILE__ ) );
			defined( 'FD_PAGE_BASENAME' ) or define( 'FD_PAGE_BASENAME', 'freshdesk-menu-handle' );
			defined( 'DOMAIN_REGEX' ) or define( 'DOMAIN_REGEX', '/\bhttps?:\/\/([-a-z0-9+&@#\/%?=~_|!:,.;]*[-a-z0-9+&@#\/%=~_|])/i' );

			// include freshdesk api class.
			//Sso handler and comment action handler
			add_action( 'init', [ $this, 'fd_login']);
			//Plugin Menu
			...
			// admin messages hook!
			add_action( 'admin_notices', [ $this, 'freshdesk_admin_msgs' ]);
		...
		/**
		* Function Name: show_login_message
		* @return void
		*/
		public function show_login_message() {

			if ( !empty($this->freshdesk_options) && $this->freshdesk_options['freshdesk_enable_sso'] != 'checked' && isset( $_GET['action'] ) && $_GET['action'] == 'freshdesk-login' ){
		...
		/**
		* Function Name: fd_login_redirect
		* @return void
		*/
		public function fd_login_redirect( $url, $request, $user ) {
			...
			$redirect_url = $this->get_redirect_url( $fd_redirect_to );
			...
			if ( isset( $_REQUEST['wp-submit'] ) && $_REQUEST['wp-submit'] == "Log In" && is_a( $user, 'WP_User' ) && $redirect_url ) {
				...
				$ssl_url = $this->freshdesk_sso_login_url( $redirect_url, $user_name, $user_email, $secret);
		...
		/**
		* Function Name: get_redirect_url
		* @return string
		*/
		public function get_redirect_url($host_url) {
		...
		/**
		* Function Name: freshdesk_plugin_menu
		* @return void
		*/
		...
		/**
		* Function Name: freshdesk_settings_page
		* @return void
		*/
		...
		/**
		* Function Name: freshdesk_settings_init
		* @return void
		*/
		public function freshdesk_settings_init() {
			...
			//Domain url.
			add_settings_field( 'freshdesk_domain_url', __('Freshdesk URL'), array($this, 'freshdesk_domain_callback') ,'freshdesk-menu-handle', 'freshdesk_settings_section' );
			//Host url
		...
		/**
		* Function Name: freshdesk_domain_callback
		...
		*/
		...
		/**
		* Function Name: freshdesk_cname_callback
		* @return void
		*/
		public function freshdesk_cname_callback() {
			echo "<input class='fd_ui_element' id='freshdesk_cname' name='freshdesk_options[freshdesk_cname]' size='72' type='text' value='{$this->freshdesk_options['freshdesk_cname']}' />";
			echo '<div class="info-data fd_ui_element">Eg: https://support.yourdomain.com,https://support2.yourdomain.com</div>';
		}

		/**
		* Function Name: freshdesk_enable_sso_callback
		* @return void
		*/
		public function freshdesk_enable_sso_callback() {
			...
			$this->freshdesk_sso_callback();
			
			//Remote login and Logout Urls
			$this->freshdesk_sso_urls_callback();
			echo '</div>';
		...
		*/
		public function freshdesk_sso_callback() {
			echo "<br><div class='freshdesk_sso_settings' style='display: none;' ><div class='info-title'>".__("Simple SSO shared secret")."</div><input class='fd_ui_element' id='freshdesk_sso_key' name='freshdesk_options[freshdesk_sso_key]' size='72' type='text' value='{$this->freshdesk_options['freshdesk_sso_key']}' />";
			echo '<div class="info-data fd_ui_element freshdesk_helpdesk_url">Enable SSO in your Freshdesk account, copy the <a href="{$this->freshdesk_options["freshdesk_domain_url"]}/admin/security" target="_blank" >shared secret from this page</a> and paste it here.</div></div>';
		}

		/**
		* Function Name: freshdesk_sso_urls_callback
		* @return void
		*/
		public function freshdesk_sso_urls_callback() {
			...
			echo '<div class="info-data freshdesk_helpdesk_url">'.__("Copy the <i>Remote login URL</i> to your ").'Freshdesk Simple SSO settings <a href="{$this->freshdesk_options["freshdesk_domain_url"]}/admin/security" target="_blank" >in this page.</a></div></li>';
			...
			echo '<div class="info-data freshdesk_helpdesk_url" id="freshdesk_redirect_url">'.__("Copy the <i>Remote logout URL</i> to your ").'Freshdesk simple SSO settings <a href="{$this->freshdesk_options["freshdesk_domain_url"]}/admin/security" target="_blank" >in this page.</a></div></li></ul>';
		}

		/**
		* Function Name: freshdesk_api_callback
		* @return void
		*/
		...
		/**
		* Function Name: freshdesk_enable_fb_callback
		* @return void
		*/
		...
		/**
		* Function Name: freshdesk_fb_widget_callback
		* @return void
		*/
		...
		/**
		* Function Name: validate_freshdesk_settings
		...
		*/
		...
		/**
		* Function Name: validate_freshdesk_fb_settings
		* @param $input
		* @return array
		*/
		...
		/**
		* Function Name: validate_checkbox
		* @param $input
		* @return string
		*/
		...
		/**
		* Function Name: freshdesk_comment_action
		...
		public function fd_login() {
			...
			if ( 'wp-login.php' == $pagenow ){
				...
				if ( $_GET['action'] == 'freshdesk-login' ) {
					...
					if ( is_user_logged_in() ) {
						...
						$url = $this->freshdesk_sso_login_url( $this->get_redirect_url($domain), $user_name, $user_email, $secret);
						header( 'Location: '.$url ) ;
						die();
					}
				...
				if ( $_GET['action'] == 'freshdesk-logout' ) {
					wp_logout();
					header( 'Location: '.$this->get_redirect_url($domain) );
					die();
				}
			}
			...
			if ( $pagenow == 'edit-comments.php' ||  ( isset( $_GET['page'] ) && $_GET['page'] == 'freshdesk-menu-handle' ) ) {
				if ( current_user_can( 'manage_options' ) ) {
					wp_enqueue_script( 'fd_plugin_js', FD_PLUGIN_URL . 'public/js/freshdesk_plugin_js.js', array( 'jquery' ) );
					wp_localize_script( 'fd_plugin_js', 'freshdeskAjax', array( 'ajaxurl' => admin_url( 'admin-ajax.php' ) ) );
				}
			}

			wp_enqueue_style( 'fd_plugin_css', FD_PLUGIN_URL . 'public/css/freshdesk_plugin.css' );
			
			$feedback_options=get_option( 'freshdesk_feedback_options' );
			if ( $feedback_options['freshdesk_enable_feedback'] == "checked" ) {
				add_action( 'wp_footer', array($this, 'freshdesk_widget_code' ));
			}
		}

		/**
		* Function Name: freshdesk_widget_code
		* Feedback widget code snippet include.
		* @return void
		*/
		public function freshdesk_widget_code() {
			$options = get_option( 'freshdesk_feedback_options' );
			echo $options['freshdesk_fb_widget_code'];
		}

		/**
		* Function Name: freshdesk_sso_login_url
		* @param $domain, $user_name, $email, $secret
		* @return string
		*/
		public function freshdesk_sso_login_url($domain, $user_name, $email, $secret){
			$time = time();
		...
		*/
		...
		/**
		* Function Name: freshdesk_show_msg
		* @param $message, $msgclass
		* @return void
		*/
		public function freshdesk_show_msg( $message, $msgclass = 'info' ) {
			echo "<div id='message' class='$msgclass'>$message</div>";
		}

		/**
		* Function Name: freshdesk_admin_msgs
		* @return void
		*/
		public function freshdesk_admin_msgs() {
			...
			if ( current_user_can ('manage_options') && $wptuts_settings_pg !== FALSE && ! empty( $set_errors ) ){
				...
				if ( $set_errors[0]['code'] == 'settings_updated' && isset( $_GET['settings-updated'] ) ) {
					$this->freshdesk_show_msg("<p>" . $set_errors[0]['message'] . "</p>", 'updated');
				} else {
					// there maybe more than one so run a foreach loop.
					foreach ( $set_errors as $set_error ) { // start of the $set_errors loop
						// set the title attribute to match the error "setting title" - need this in js file
						$this->freshdesk_show_msg("<p class='setting-error-message' title='" . $set_error['setting'] . "'>" . $set_error['message'] . "</p>", 'error');
...
/*
* Initialising Fresh_Desk instance
*/
new Fresh_Desk();